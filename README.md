...
| Matrix engines | Four engines per CU; wave32; M16N16K16 FP16/BF16 and M16N16K32 FP8/INT8; fixed fragment map, 16-cycle issue target, bank-safe control RTL, pending-destination interlocks, and capture/active operand staging | [Matrix Engine Architecture](docs/MATRIX_ENGINE.md) |
...
A standard CMake toolchain is sufficient for the current analytical model, ISA reference tests, matrix numeric, physical-architecture, register-interface schedule, bank-conflict, and operand-staging tests, power-management policy tests, and firmware tests.
