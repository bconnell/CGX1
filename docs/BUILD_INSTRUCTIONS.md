...
- matrix numeric behavior, tile shape, wave32 fragment mapping, register contract, capture/writeback schedule, modulo-8 bank-class placement, capture/active operand staging, cycle-0 output bypass, output-result staging, per-wave scoreboard behavior, signed INT8 arithmetic/path integration, one-wave INT8 engine-shell behavior, Matrix opcodes, reduction order, issue interval, and latency against [Matrix Engine Architecture](MATRIX_ENGINE.md);
...
The public SystemVerilog directory contains limited control/storage/arithmetic RTL for board power state behavior, matrix pipeline scheduling, matrix staging, per-wave VGPR hazard tracking, signed INT8 matrix execution, and a one-wave INT8 engine shell. It is not full GPU RTL. Run `./scripts/validate_rtl.sh` with Icarus Verilog installed to execute the current matrix control testbench.
