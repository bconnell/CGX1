...
The public SystemVerilog directory contains limited control/storage/arithmetic RTL for board power state behavior, matrix pipeline scheduling, matrix staging, per-wave VGPR hazard tracking, and signed INT8 matrix execution. It is not full GPU RTL. Run `./scripts/validate_rtl.sh` with Icarus Verilog installed to execute the current matrix control testbench.
