...
- matrix numeric behavior against [Matrix Engine Precision and Semantics](MATRIX_ENGINE.md), while leaving physical tile shape and throughput unfrozen until implementation evidence exists;
...
1. Implement scalar/vector issue, register files, arithmetic, matrix engines matching the frozen numeric contract, load/store, texture, ray traversal, raster functions, and local caches.
