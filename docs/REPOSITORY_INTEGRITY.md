...
The check includes card dimensions, dock dimensions, selected card fan thickness, power states, FP32 arithmetic target, memory capacity, memory bandwidth, matrix-engine count, matrix numeric-policy flags, frozen matrix tile shapes, register contract, issue timing, latency, and theoretical dense-rate targets.
