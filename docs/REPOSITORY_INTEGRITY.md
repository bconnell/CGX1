...
The check includes card dimensions, dock dimensions, selected card fan thickness, power states, FP32 arithmetic target, memory capacity, memory bandwidth, matrix-engine count, matrix numeric-policy flags, frozen matrix tile shapes, register contract, issue timing, latency, whole-wave capture/writeback interface requirements, staging capacity, and theoretical dense-rate targets.
