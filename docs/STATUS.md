...
| RTL | Limited control/storage/arithmetic RTL | Power-state/tile-enable scaffold, matrix control, staging, per-wave VGPR scoreboard, signed INT8 arithmetic, validated INT8 end-to-end path, and a one-wave INT8 engine shell validated by the repository RTL simulation gate; FP16/BF16/FP8 arithmetic and physical implementation remain open |
