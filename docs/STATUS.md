...
| RTL | Limited control/storage RTL + executable simulation | Power-state/tile-enable scaffold, matrix pipeline controller, capture/active operand staging, output-result staging, and per-wave matrix/ordinary VGPR scoreboard; arithmetic datapath, physical storage macros, ordinary vector execution integration, and multi-wave scheduling remain open |
