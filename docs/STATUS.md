...
| Scheduling/preemption | Defined workgroup-boundary architecture; complete-residency/barrier reference and candidate RTL locally tested; full CU composition remains open | [Scheduling and Preemption](SCHEDULING_PREEMPTION.md), [Validation](VALIDATION.md) |
...
| RTL | Limited control/storage/arithmetic and scheduling RTL | Power-state/tile-enable scaffold, matrix control/staging, signed INT8 arithmetic/path, validated one-wave shell, parameterized resident-wave arbitration/dependencies, pooled VGPR path, mixed matrix/vector frontend, and a parameterized complete-workgroup admission/barrier boundary with local tests; full CU integration, FP16/BF16/FP8 arithmetic, and physical implementation remain open |
...
A new executable CU workgroup reference and parameterized RTL boundary enforce atomic admission of all waves and reservations for resident slots, pooled-VGPR rows, scalar/predicate state, shared/local-memory capacity, barrier contexts, and additional local state. Barrier waits retain wave residency and only block arrived participants. Normal wave retirement or fault removes that participant; whole-workgroup abort and reset release all ownership. The RTL test drives the issuable-wave mask into the existing resident vector arbiter. This local slice does not yet compose the new row reservations with the pooled VGPR execution subsystem or connect the controller to the mixed matrix/vector frontend, real shared/local-memory access, memory waits, full dispatch, fault reporting, or retirement. Exact-revision CI simulation and physical evidence remain pending.
