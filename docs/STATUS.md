...
| RTL | Limited control RTL + executable simulation | Power-state/tile-enable scaffold plus matrix pipeline controller with bank-safe addressing and matrix-to-matrix pending-destination interlocks; arithmetic datapath, physical register/staging implementation, and general compute-unit scoreboard remain open |
