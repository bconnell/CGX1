...
The mixed resident execution frontend has a dedicated executable admission reference and SystemVerilog integration test. The test covers pooled initialization, resident vector execution/writeback, release blocking while vector execution is live, matrix blocking on a live vector destination hazard, recovery after that hazard clears, and same-edge matrix/vector acceptance exclusion. The frontend derives vector/matrix dependencies from the internal per-wave matrix scoreboard; a full compute-unit scheduler for memory waits, barriers, faults, dispatch, and retirement remains unimplemented.

The workgroup scheduler reference test and `cgx1_workgroup_residency_barrier_tb.sv` exercise complete-workgroup residency as the barrier-progress baseline. Coverage includes one-wave and simultaneous barriers, staggered and arbitrary arrival order, a nonarrived sibling continuing through the existing resident-vector issue arbiter, independent workgroups, repeated barrier generations, maximum-fit and oversized wave/VGPR/scalar/shared-memory demands, pooled-row fragmentation, scalar/shared-memory/other-state/barrier-context aggregate-capacity failures, completion/release, retirement or fault of a wave already at the barrier, whole-workgroup kill, reset while blocked, and randomized forward progress (100,000 reference transitions and 5,000 RTL cycles). Assertions check that every live admitted wave remains resident and that each wave is either issuable or waiting at a barrier. The standalone controller reserves a logical first-fit VGPR-row map; composition with `cgx1_pooled_vgpr_execution_subsystem`, the mixed frontend, actual shared/local-memory access, memory waits, full dispatch/fault/retirement, and whole-workgroup preemption handling remain open. Local C++ and Icarus passes are not exact-revision CI evidence or physical implementation evidence.
...
The public RTL currently covers top-level power-state/tile-enable behavior, matrix pipeline control, capture/active operand staging, output-result staging, signed INT8 arithmetic/path, one-wave hazard reporting, parameterized resident-wave matrix arbitration, wave-local dependency handling, per-wave scoreboard routing, pooled VGPR lifecycle/storage, ordinary INT32 vector execution, and a mixed matrix/vector frontend. The current local working-tree candidate passed `scripts/validate_rtl.sh` with Icarus 12.0 in Ubuntu WSL. The C/C++ Release build and 21 executable tests passed under GCC 13.3 in WSL. The Windows validation wrapper's CMake step remains unavailable because `cmake.exe` is absent from Windows PATH; repository hygiene, negative controls, design consistency, and Markdown-link checks passed before that environment failure. This evidence is local and does not establish published CI, formal verification, synthesis, timing, area, power, physical implementation, or silicon behavior. FP16/BF16/FP8 arithmetic, foundry storage-macro selection, the complete compute-unit scheduler, and the rest of the GPU pipeline remain open.
