...
- `cgx1_vector_resident_wave_scheduler.sv` and `cgx1_resident_vector_execution_frontend.sv` provide parameterized resident-wave vector selection and payload routing.
...
The matrix RTL implements signed INT8 arithmetic only. FP16, BF16, and FP8 arithmetic remain unimplemented because their contract requires FP32 fused multiply-add semantics. Resident-wave matrix identity/arbitration now coexists with a candidate resident-wave INT32 vector execution path and shared pooled-VGPR arbitration. The earlier banked per-slot VGPR storage boundary has passed RTL simulation. A pooled resident-wave allocation/storage path is now published and connected to the resident signed INT8 engine, with behavioral testbenches wired into the RTL workflow. Exact-revision simulation evidence for the pooled boundary remains pending until the current revision passes that workflow. No foundry memory macro, timing, area, or power claim is made. The published INT32 vector path is now composed with the resident signed INT8 matrix engine in a candidate mixed execution frontend. Its per-wave vector dependency-ready input is still supplied externally, so canonical compute-unit scheduling, scalar/control execution, memory execution, and physical implementation remain open.
