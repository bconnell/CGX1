...
module cgx1_compute_int8_vector_execution_frontend #(
 ...
 input logic[RESIDENT_WAVE_SLOTS-1:0]vector_request_valid,vector_dependency_ready,
 ...
 cgx1_resident_vector_execution_frontend #(.RESIDENT_WAVE_SLOTS(RESIDENT_WAVE_SLOTS),.WAVE_SLOT_WIDTH(WAVE_SLOT_WIDTH)) vsched(
  .clk(clk),.reset_n(reset_n),.request_valid(vector_request_valid),.dependency_ready(vector_dependency_ready),.request_opcode(vector_request_opcode),.request_source0(vector_request_source0),.request_source1(vector_request_source1),.request_destination(vector_request_destination),.request_lane_mask(vector_request_lane_mask),.request_accepted(vector_request_accepted),.selected_valid(vec_selected_valid),.selected_wave_slot(vec_selected_wave),.selected_opcode(vec_selected_opcode),.selected_source0(vec_selected_s0),.selected_source1(vec_selected_s1),.selected_destination(vec_selected_d),.selected_lane_mask(vec_selected_mask),.selected_ready(vec_selected_ready));
 ...
 always_comb begin
   ...
   for(mi=0;mi<RESIDENT_WAVE_SLOTS;mi=mi+1) begin
     engine_matrix_valid[mi]=matrix_request_valid[mi] && (!matrix_request_full_wave_active[mi] || !layout_legal[mi] || (preflight_ready[mi]&&matrix_vector_ready[mi]));
     matrix_request_ready[mi]=engine_matrix_ready[mi] && (!matrix_request_full_wave_active[mi] || !layout_legal[mi] || (preflight_ready[mi]&&matrix_vector_ready[mi]));
   end
 end
