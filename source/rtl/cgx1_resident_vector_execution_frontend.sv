...
module cgx1_resident_vector_execution_frontend #(
 ...
 logic scheduler_accepted;
 cgx1_vector_resident_wave_scheduler #(.RESIDENT_WAVE_SLOTS(RESIDENT_WAVE_SLOTS),.WAVE_SLOT_WIDTH(WAVE_SLOT_WIDTH)) scheduler(
  .clk(clk),.reset_n(reset_n),.request_valid(request_valid),.dependency_ready(dependency_ready),.selected_valid(selected_valid),.selected_wave_slot(selected_wave_slot),.selected_accepted(scheduler_accepted));
 always_comb begin
   ...
   scheduler_accepted=selected_valid&&selected_ready;
 end
endmodule
