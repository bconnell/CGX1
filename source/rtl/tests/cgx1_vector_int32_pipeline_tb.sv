...
module cgx1_vector_int32_pipeline_tb;
 ...
 logic complete_valid;logic[1:0]complete_wave_slot;logic illegal_opcode,address_fault,uninitialized_fault,busy;logic[1:0]live_wave_slot;logic[7:0]live_source0,live_source1,live_destination;logic source_locks_live,destination_lock_live;
 always #5 clk=~clk;
 ...
 initial begin
  ...
  issue_source0=8'hff;issue_source1=8'hfe;issue_destination=8'hfd;issue(4'h8);#1;if(!complete_valid||!illegal_opcode||read_valid||write_valid||address_fault||uninitialized_fault||source_locks_live||destination_lock_live)$fatal(1,"illegal vector opcode waited for operand service or acquired VGPR locks");
